...
router.post('/search', async (req, res) => {
  try {
    ...
    console.log('✅ Card found:', data.name, '| Balance:', data.balance);
...
router.post('/deduct', async (req, res) => {
  try {
    ...
    console.log('💳 Deduct Request:', { rfid_uid, amount: parsedAmount, reason });
...
router.post('/topup', async (req, res) => {
  try {
    ...
    console.log('💰 Topup Request:', { rfid_uid, amount: parsedAmount });
...
router.get('/:rfidUID', async (req, res) => {
  try {
    ...
    console.log('🔍 Get card info:', rfidUID);
...
// 5. POST /api/rfid/pos-notify
// ESP32 calls this to notify POS about card tap
router.post('/pos-notify', async (req, res) => {
  try {
    const { rfid_uid, status, message, balance, timestamp } = req.body;

    console.log('📡 ESP32 → POS Notification:', {
      rfid_uid,
      status,
      message,
      balance,
    ...
    // TODO: Store this in cache or broadcast to connected POS clients
    // For now, just acknowledge receipt
    
    res.json({
      success: true,
      received: true,
      message: 'POS notified successfully',
      rfid_uid: rfid_uid
    });
